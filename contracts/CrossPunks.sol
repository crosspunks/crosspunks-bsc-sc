 ...
 */
contract CrossPunks is Ownable, ERC165, IERC721Metadata {
    ...
    string public constant CROSSPUNKS_PROVENANCE = "ffa993388253d151e96ed2d68f9ed78b3f1ac2bc6e2c5cf5041d30897dd2943d";
    ...
    function startAirDrop() public returns (uint256){
        ...
        usersAirdropAddress[airDropId] = msg.sender;
        airDropId++;
    ...
    function initializeOwners(address[] memory users, uint256 _column) public onlyOwner {
        ...
        for(uint256 i=0; i<users.length;i++){
            uint256 pIndex = ((_punksIndex[_column]) + ((_column * _punksPerColum)));
    ...
    * @dev Mints CrossPunk
    */
    function mintNFTAirDrop(uint256 numberOfNfts, uint256 _airDropId) public payable {
        require(_startSale, "Sale is not start");
        require(totalSupply() < MAX_NFT_SUPPLY, "Sale has already ended");
        require(numberOfNfts > 0, "numberOfNfts cannot be 0");
        require(numberOfNfts <= 20, "You may not buy more than 20 NFTs at once");
                ...
                for (uint256 i=0; i<5; i++) {
                    ...
                    if (usersAirdrop[wads].isExists) {
                        if (wads == _fads) {
                            winners[i] = ads;
                            break;
                        } else if(usersAirdrop[ads].referralBuyIndex > usersAirdrop[wads].referralBuyIndex) {
                            winners[i] = ads;
                            ads = wads;
                        }  
                    } else {
